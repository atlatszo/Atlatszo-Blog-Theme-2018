...
//if (get_post_type(get_the_ID())!="nav_menu_item") {return;} // only posts

  /*$temp_dir=get_template_directory_uri();
  $file="mainmenu.html";
  $menu_contents = file_get_contents($temp_dir.'/headermain.php');
  $menu_contents.="menu here";
  file_put_contents($file, $menu_contents);*/

  $file=get_home_path()."mainmenu.html";
  $menu_contents = file_get_contents('https://dummy.atlatszo.hu/menu-generator/');
  file_put_contents($file, $menu_contents);

  //var_dump($menu_id);
  
  //include_once($temp_dir.'/headermain.php');



    /*ob_start();
    ÷$content="";
    include($temp_dir.'/headermain.php');
    $content.=ob_get_clean();
    //var_dump($content);*/
